...
function initMap() {
    ...
    xmlhttp.onreadystatechange = function() {
	 ...
	 //document.getElementById("id01").innerHTML = myArr;
       ...
       totalDeath = json.data[9][11];
      
       totalYouthDeath = json.data[9][17];
       //alert(totalDeath);
       //alert(totalYouthDeath);
       //alert(washedData);
	        ...
	        if (status === google.maps.ElevationStatus.OK) {
			  ...
			  google.maps.event.addListener(markers[key],'click',function(){
			     ...
			     document.getElementById("DeathYouth").innerHTML = "<b>Death toll all causes age 25-44 all year</b>: <em>" + deadYouth + "</em>";
                            ...
                            var d = [
                                [
                                    {axis:"Death All",value:array[0]},
                                    {axis:"Death Youth",value:array[1]}
                                   
                                 
                                ]
                            ];

                            document.getElementById("scores").innerHTML = "The dangerousness for this city is <b><em>" + parseInt(score(array)*100) + "</b></em> out of <b><em>100</b></em>";   
...
function score(data) {
    var percentOfAll;
    if (data[0] != -1) {
      percentOfAll = data[0] / (totalDeath/122);
    }
   
    var percentOfYouth;
    if (data[1] != -1) {
      percentOfYouth = data[1] / (totalYouthDeath/122);
    }

    if (data[0] == -1 && data[1] != -1) {
      return percentOfYouth;
    }
    ...
    return (percentOfAll)*0.8 + (percentOfYouth)*0.2;
}
